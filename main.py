...
if __name__ == "__main__":
    ...
    for i in range(27, n - TRAINING_CHUNKS - TEST_CHUNKS, TEST_CHUNKS):
        print(" ")
        print("##############################################")
        ...
        constants.Genetic_Algorithm
            .Builder()
            .set_population_size(best_params[0])
            .set_top_pick(best_params[1])
            .set_wheel_of_fortune(best_params[2])
            .set_random_individual(best_params[3])
            .set_mutation_probability(best_params[4])
            .set_crossover_rate(best_params[5])
            .set_no_improvement(best_params[6])
            .set_min_improvement(best_params[7])
            .build()
        )
        ...
        individuals_performance_long = run_backtest.run_backtest(population_long[0:best_params[1]], test_data, False, test_emas_data_warmup)
        individuals_performance_short = run_backtest.run_backtest(population_short[0:best_params[1]], test_data, True, test_emas_data_warmup)
        ...
        average_return_long = np.sum(returns_long * (1 / best_params[1]))
        average_return_short= np.sum(returns_short * (1 / best_params[1]))
        ...
        with open("results.txt", "a") as file:
            file.write(f"Return: {average_return_total}, Index Chunks: {i}\n")
